...
def viz_training_data(tot_data, tot_lab, tot_ideal_ind, types, emb, emb_type, cmap_dict, title, fig_size, leg_adjust):
    ...
    for g in np.unique(all_labels):
        ...
        if g == -1:
            ...
            ax.scatter(scatter_x[i], scatter_y[i], label=types[int(g)], c=cmap_dict[types[int(g)]], s=6)
...
def cell_type_classifier(ideal_labels, ideal_cells, test_cells, train_ind, learning_rate, training_epochs, batch_size, display_step):
    ...
    # converting train, validation, and test obj to dense if stored as sparse
    if scipy.sparse.issparse(training_cells):
        training_cells = training_cells.todense()
    if scipy.sparse.issparse(valid_cells):
        valid_cells = valid_cells.todense()
    if scipy.sparse.issparse(test_cells):
        test_cells = test_cells.todense()
    ...
    with tf.Session() as sess:
        ...
        true1 = sess.run(pred, feed_dict={x: test_cells})  # need to evaluate the readout here
